...
int main(){
	cin>>num_houses>>num_candidate>>num_roads>>range;
	...
	for(int i=num_houses+1;i<=num_candidate+num_candidate;i++){//interating through all candidates
		bool valid=true;	
		fill(&visited[0],&visited[0]+maxn,false);
		fill(&dis[0],&dis[0]+maxn,inf);
		dis[i]=0;
		int sum=0,sample_min_dis=inf,next=-1;
		for(int j=0;j<num_houses+num_candidate;j++){//Dijstra
			int min=inf,next=-1;
			for(int k=1;k<=num_houses+num_candidate;k++){
			...
			//printf("G%d sample_min_dis:%d total:%d \n",i-num_houses,sample_min_dis,dis);
	...
	printf("G%d\n%.1f %.1f",res-num_houses,(float)res_min_dis,(float)res_sum/num_houses+0.001);
}